...
;(function(window, jQuery, undefined) {
	...
	(function ( $ ) {
	    ...
	    $.fn.shareSocial = function( options ) {
	        ...
	        function buttonText(website) {
	        	if(settings.showText) {
	        		if (settings.shareText.length > 0) {
			            var customText = settings.shareText.replace(settings.shareText, encodeURIComponent(settings.shareText));
			            return '<span class="text">'+customText+'</span>';
			        } else {
				        return '<span class="text">'+website+'</span>';
				    };
	        	};
			};
			var icons = {
	        ...
	        this.children().each(function(index, el){
	        	var website = $(el).attr('data-website');
	        	if(settings.shareIcon) {
				    var buttonIcon = '<span class="icon">'+icons[website]+'</span>';
	        	} else {
	        		return buttonIcon = '';
	        	};
	        	if (settings.sharePopup) {
		            var hasPopup = 'popup';
		        } else {
		        	return hasPopup = '';
		        };
	        	console.log(website+'  ');
	        	//$(el).addClass(website).append($('<a href="'+apiLink(website)+'">'+buttonIcon+buttonText(website)+'</a>').addClass(hasPopup));
	        	$('<a href="'+apiLink(website)+'">'+buttonIcon+buttonText(website)+'</a>').addClass(hasPopup).appendTo($(el).addClass(website));
